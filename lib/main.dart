...
Uint8List wavFileHeader(int soundDataLen) {
  ...
  bd.setUint32(32, 0x02001000, Endian.big); // 2=BlockAlign       == NumChannels * BitsPerSample/8;  16 = BitsPerSample
...
// ffmpeg -i NicoA1.webm -t 1 -ar 8000 -ac 1 a.wav
    ...
    for (int i = 0; i < 8000; i++) {
      byteData2.setInt16(
          i * 2, (50 + sin(i) * 50).floor() * 256, Endian.little);
      // data.add(0);
    ...
    return StreamAudioResponse(
        sourceLength: data.length + 8000 * 2,
        contentLength: data.length + 8000 * 2,
        offset: 0,
        //stream: Stream.value(data),
        stream: Stream.value(b.toBytes()),
